...
class FEModel():
    def __init__(self, L=4, storeys=3, qd=3.6):
        ...
        for cs in ('beam', 'col'):
            ...
            self.crosSecs[cs]['I0'] = 2 * (self.crosSecs[cs]['bf'] * self.crosSecs[cs]['tf'] ** 3 / 12 +                                 self.crosSecs[cs]['bf'] * self.crosSecs[cs]['tf'] * (self.crosSecs[cs]['hw'] / 2 + self.crosSecs[cs]['tf'] / 2) ** 2) +                                 self.crosSecs[cs]['tw'] * self.crosSecs[cs]['hw'] ** 3 / 12
...
class frameEnv(Env):
    ...
    def step(self, action, fixed_obs, num_samples):
        ...
        self._variational_inference(fixed_obs, num_samples)
        ...
        return np.concatenate((self.shapes, self.ages)), reward, done, info, self.vi_trace
    ...
    def _variational_inference(self, fixed_obs, num_samples, draws=4000, tune=2000, targetAccept=0.9):
        """
        Performs the Bayesian Inference, i.e. updates the distributions of parameters A, B,
        using the damage increments that are obtained using a Gamma process
        """
        with pm.Model():
            ...
            if self.decisionStep == 0:
                ...
                samples = self.vi_trace.sample(1000)
                A = self._from_posterior('A', samples.get_values("A"))
                B = self._from_posterior('B', samples.get_values("B"))
            ...
            advi = pm.ADVI()
            self.vi_trace = advi.fit(n=num_samples, progressbar=False)
            #az.plot_posterior(self.vi_trace.sample(1000), color="LightSeaGreen")
    ...
    def _from_posterior_vi(self, param, samples):
        """
        Transforms the posterior distributions to priors for the next iteration inference
...
if __name__ == "__main__":
    ...
    file = args[0]
    samples = args[1]
    muA, muB, noise, observations = create_from_file(file)
    ...
    while not done:
        ...
        action = np.random.choice([0], size=(6,))
        ...
        try:
            state_, reward, done, info, trace = env.step(action, observations, int(samples))
    ...
    a_samples = samples.get_values("A")
    b_samples = samples.get_values("B")
    a_total = 0
    b_total = 0
    for i in range(1000):
        a_total += a_samples[i]
    ...
    print("muA ", a_total/1000, " muB ", b_total/1000)
